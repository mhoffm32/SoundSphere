...
document
  .getElementById("past-lists")
  .addEventListener("click", display_past_lists);
    ...
    .then((data) => {
      data.forEach((list, index) => {
        if (list.name.toLowerCase() == listName.toLowerCase()) {
          ...
          listIds.forEach((id) => {
            id = parseInt(id, 10);
            const fetchHeroData = fetch(`/api/hero/${id}`).then((res) =>
              res.json()
            );
            const fetchPowersData = fetch(`/api/hero/${id}/powers`).then(
              (res) => res.json()
            );

            Promise.all([fetchHeroData, fetchPowersData]).then(
              ([heroData, powersData]) => {
                const infoAndPowers = `Hero Name: ${heroData.name}, Gender: ${
                  heroData.Gender
                }, Eye Color: ${heroData["Eye color"]}, Race: ${
                  heroData.Race
                }, Hair Color: ${heroData["Hair color"]}, Height: ${
                  heroData.Height
                }, Publisher: ${heroData.Publisher}, Skin Color: ${
                  heroData["Skin color"]
                }, Alignment: ${heroData.Alignment}, Weight: ${
                  heroData.Weight
                }, Powers: [${powersData.join(", ")}]`;
...
function get_past_lists() {
  fetch("/api/hero/past_lists").then((res) => {
    res.json().then((data) => {
      if (data) {
        return true;
      }
    ...
    .then((data) => {
      data.forEach((list, index) => {
        ...
        listIds.forEach((id) => {
          ...
          const fetchPowersData = fetch(`/api/hero/${id}/powers`).then((res) =>
            res.json()
          );
...
function load_data() {
  fetch("/load-superhero-data").then((res) => {
    res.json().then((data) => {
...
function searchHeroes() {
  ...
  fetch(`/api/hero/search/${property}/${property_value.value}/${sortBy}`).then(
    (res) => {
      res.json().then((data) => {
        ...
        data.forEach((element) => {
          ...
          item.appendChild(
            document.createTextNode(
              `ID: ${element.id}, Name:${element.name}, Gender: ${element.Gender}, Eye Color: ${element["Eye color"]}, Race: ${element.Race}, Hair Color: ${element["Hair color"]}, Height: ${element.Height}, Publisher: ${element.Publisher}, Skin Color: ${element["Skin color"]}, Alignment: ${element.Alignment}, Weight: ${element.Weight}, Powers: [${powers_display}]`
            )
          );
          l.appendChild(item);
...
function getById(id_input) {
  fetch(`/api/hero/${id_input}`).then((res) => {
...
function getPowers(id) {
  fetch(`/api/hero/${id}/powers`).then((res) => {
    res.json().then((powers) => {
      console.log(powers);
      return powers;
    });
  });
...
function getPublishers() {
  fetch(`/api/hero/${property}/powers`).then((res) => {
    res.json().then((data) => {
      const l = document.getElementById("searchResults");
...
function getMatches() {
  ...
  if (isNaN(n_input.value)) {
    ...
    document.getElementById("match-message").textContent =
      "Please Enter a valid integer for n";
    return;
  }
  if (!n_input.value) {
    document.getElementById("match-message").textContent =
    ...
    if (
      fieldSelector.value.trim() == "Height" ||
      fieldSelector.value.trim() == "Weight"
    ) {
    ...
    n = n_input.value;
    fetch(`/api/hero/${field}/${pattern}/${n}`).then((res) => {
      res.json().then((data) => {
        if (data.length == 0) {
          document.getElementById("match-message").textContent =
            "No matches found.";
...
function getPatternBtns() {
  ...
  document.querySelector(`label[for="${"select-pattern"}"]`).style.display =
    "inline-block";
  document.getElementById("match-message").textContent = "";
  ...
  if (selected_field === "Height" || selected_field === "Weight") {
    ...
    fetch(`/api/hero/search/${selected_field}`).then((res) => {
      res.json().then((patterns) => {
