...
const HeroSearch = ({ choice }) => {
  ...
  const fillOptions = async () => {
    try {
      ...
      if (!response.ok) {
        console.error(
          `Request to fetch publishers failed with status ${response.status}: ${response.statusText}`
        );
      } else {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `Request to fetch powers failed with status ${response.status}: ${response.statusText}`
        );
      } else {
        setHPowers(powersL);
      }
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `Request to fetch races failed with status ${response.status}: ${response.statusText}`
        );
      } else {
        setHRaces(racesL);
      }
  ...
  const getResults = async () => {
    ...
    try {
      const response = await fetch(
        ...
        }/${encodeURIComponent(selectedRace.trim())}/${encodeURIComponent(
          selectedPower.trim()
        )}/${encodeURIComponent(selectedPub.trim())}`
      );
    ...
    <div className="hero-search">
      ...
      {hero_races !== null && hero_races !== undefined && (
        <span>
          ...
          >
            <option id="no-sort" key="none" value="none">
              None
            </option>
            {hero_races.map((race) => (
